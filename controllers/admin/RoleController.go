...
func (c *RoleController) DataGrid() {
	...
	var params admin.RoleQueryParam
	json.Unmarshal(c.Ctx.Input.RequestBody, &params)
	//获取数据列表和总数
	data, total := admin.RolePageList(&params)
	//定义返回的数据结构
...
func (c *RoleController) DataList() {
	var params = admin.RoleQueryParam{}
	//获取数据列表和总数
	data := admin.RoleDataList(&params)
	//定义返回的数据结构
...
func (c *RoleController) Delete() {
	...
	if num, err := admin.RoleBatchDelete(ids); err == nil {
...
func (c *RoleController) Allocate() {
	...
	o := orm.NewOrm()
	m := admin.Role{Id: roleId}
	if err := o.Read(&m); err != nil {
		c.jsonResult(enums.JRCodeFailed, "数据无效，请刷新后重试", "")
	}
	//删除已关联的历史数据
	if _, err := o.QueryTable(admin.RoleResourceRelTBName()).Filter("role__id", m.Id).Delete(); err != nil {
		c.jsonResult(enums.JRCodeFailed, "删除历史关系失败", "")
	}
	var relations []admin.RoleResourceRel
	for _, str := range strings.Split(strs, ",") {
		if id, err := strconv.Atoi(str); err == nil {
			r := admin.Resource{Id: id}
			relation := admin.RoleResourceRel{Role: &m, Resource: &r}
