...
func (c *CourseController) DataGrid() {
	...
	var params admin.CourseQueryParam
	json.Unmarshal(c.Ctx.Input.RequestBody, &params)
	//获取数据列表和总数
	data, total := admin.CoursePageList(&params)
	//定义返回的数据结构
...
func (c *CourseController) Edit() {
	...
	Id, _ := c.GetInt(":id", 0)
	m := admin.Course{Id: Id, Seq: 100, StartTime: time.Now(), EndTime: time.Now()}
	if Id > 0 {
		o := orm.NewOrm()
		err := o.Read(&m)
		if err != nil {
...
func (c *CourseController) Delete() {
	...
	if num, err := admin.CourseBatchDelete(ids); err == nil {
