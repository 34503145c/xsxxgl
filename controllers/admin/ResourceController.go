...
func (c *ResourceController) UserMenuTree() {
	userid := c.curUser.Id
	//获取用户有权管理的菜单列表（包括区域）
	tree := admin.ResourceTreeGridByUserId(userid, 1)
	//转换UrlFor 2 LinkUrl
	c.UrlFor2Link(tree)
	c.jsonResult(enums.JRCodeSucc, "", tree)
}
...
func (c *ResourceController) ParentTreeGrid() {
	Id, _ := c.GetInt("id", 0)
	tree := admin.ResourceTreeGrid4Parent(Id)
	//转换UrlFor 2 LinkUrl
	c.UrlFor2Link(tree)
	c.jsonResult(enums.JRCodeSucc, "", tree)
}
...
func (c *ResourceController) UrlFor2Link(src []*admin.Resource) {
...
func (c *ResourceController) Edit() {
	...
	//获取可以成为当前节点的父节点的列表
	c.Data["parents"] = admin.ResourceTreeGrid4Parent(Id)
	//转换地址
...
func (c *ResourceController) Save() {
	...
	//获取父节点
	if parentId > 0 {
		parent, err = admin.ResourceOne(parentId)
...
func (c *ResourceController) Delete() {
	...
	query := orm.NewOrm().QueryTable(admin.ResourceTBName())
