...
type BaseController struct {
	...
	curUser        admin.BackendUser //当前用户信息
}
...
func (c *BaseController) setBackendUser2Session(userId int) error {
	m, err := admin.BackendUserOne(userId)
	if err != nil {
		return err
	}
	//获取这个用户能获取到的所有资源列表
	resourceList := admin.ResourceTreeGridByUserId(userId, 1000)
