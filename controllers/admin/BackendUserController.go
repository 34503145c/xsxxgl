...
import (
	...
	"github.com/lhtzbj12/sdrms/models/admin"
...
func (c *BackendUserController) DataGrid() {
	...
	var params admin.BackendUserQueryParam
	json.Unmarshal(c.Ctx.Input.RequestBody, &params)
	//获取数据列表和总数
	data, total := admin.BackendUserPageList(&params)
	//定义返回的数据结构
...
func (c *BackendUserController) Save() {
	...
	//删除已关联的历史数据
	if _, err := o.QueryTable(admin.RoleBackendUserRelTBName()).Filter("backenduser__id", m.Id).Delete(); err != nil {
	...
	if m.Id == 0 {
		...
		if oM, err := admin.BackendUserOne(m.Id); err != nil {
	...
	//添加关系
	var relations []admin.RoleBackendUserRel
	for _, roleId := range m.RoleIds {
		r := admin.Role{Id: roleId}
		relation := admin.RoleBackendUserRel{BackendUser: &m, Role: &r}
...
func (c *BackendUserController) Delete() {
	...
	query := orm.NewOrm().QueryTable(admin.BackendUserTBName())
